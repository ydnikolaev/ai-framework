...
Мы используем нативные CSS-переменные Telegram для автоматической адаптации к темам (Light/Dark/Colored). 
**Важно:** Мы НЕ определяем тему через JS. Все стили должны опираться на переменные.
...
### 📐 Concentric Shapes Rule

Для вложенных элементов с радиусом скругления необходимо соблюдать правило концентричности:
`Outer Radius` - `Padding` = `Inner Radius`

**Пример (Movie Card):**
- Outer Card: `rounded-3xl` (24px)
- Padding: `p-3` (12px)
- Inner Content/Poster: `rounded-md` (12px)

`24px - 12px = 12px` ✅
Если результат отрицательный или ноль — внутренний элемент должен быть без скругления (`rounded-none`).
...
- `divider: boolean` — Линия снизу (реализована через CSS `::after`)
- `chevron: boolean` — Стрелочка справа (default: false)
- `link: boolean` — Hover эффект

> **Style Note:** `AppCell` всегда имеет прозрачный фон (`bg-transparent`). Цвет фона задается родительским контейнером (`bg-primary`). Разделитель и шеврон используют один тон (`hint-color`) с разной прозрачностью.
