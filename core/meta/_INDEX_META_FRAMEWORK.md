# 🛡️ Meta Rules — Index

> **Role:** Meta-Framework Guide
> **Objective:** Rules for modifying the framework itself.
> **Context:** Read ONLY if you are changing files in `ai-framework/core/`.

---

## 📜 Принципы

1.  **Backwards Compatibility:**
    - Не ломай существующие проекты.
2.  **Modularity:**
    - Все новые части должны быть опциональными или изолированными.
3.  **Documentation First:**
    - Изменил код фреймворка -> обнови документацию.

## 📄 Файлы

| Файл | Назначение |
|------|------------|
| [adding-framework.md](adding-framework.md) | Как добавить новый фреймворк в docs/ |

---

## 🔗 Связанные

- [../../docs/](../../docs/) — Framework documentation
- [../../templates/](../../templates/) — Project templates
