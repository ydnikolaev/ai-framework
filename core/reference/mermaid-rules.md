...
### ⚠️ Совместимость (NO FANCY FEATURES)
Не используйте новые фичи Mermaid, которые могут не поддерживаться старыми превьюверами:
- ❌ **Chaining**: `A & B --> C` (Пишите `A --> C` и `B --> C` отдельно).
- ❌ **Subgraphs**: Часто ломают рендеринг, используйте с осторожностью.
- ❌ **Styling**: `style A fill:#f9f` (Лучше избегать inline-стилей).
